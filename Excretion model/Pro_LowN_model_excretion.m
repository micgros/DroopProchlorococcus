...
% last midification : Droop-model ver 15 Feb 16
% MIT equations
...
for j=[0 1 2 4 6 8 10 15 20]

% gama=fraction of heterotroph mort/resp to inorganic form
gamma_n = 0.5; 
gamma_c = 0.5; 

% epsilon - excrition fraction - percent per day!
epsilon_n = j*0.01/86400;
epsilon_c = epsilon_n;
...
% biomass N auto
b_n = b_n + autoNbiomass_excretion(V_n_max, N, k_n, x, q_n, q_n_max, q_n_min, m_star, b_n, epsilon_n);
B_n_v(i) = b_n;

% biomass C auto
b_c = b_c + autoCbiomass_excretion(V_c_max, C, k_c, x, q_c, q_c_max, q_c_min, m, b_c, r0, b, mu, epsilon_c);
B_c_v(i) = b_c;
...
% organic nutrients
ON      = ON + organicN_excretion(m_star, b_n, gamma_n, epsilon_n);
ON_v(i) = ON;
OC      = OC + organicC_excretion(m_star, b_c, gamma_c, epsilon_c);
OC_v(i) = OC;
...
% end 
save excretion X_vv -append -ascii
end
...
plot(time,excretion)
hold on
errorbar(xdot9312,ydot9312,SD9312)
