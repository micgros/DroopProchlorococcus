...
V_n_max = 10*((9.1e-9) / (ts*86400)) * vol^0.67; 
V_c_max = V_n_max; 
...
% biomass (umol N l-1 )
b_n = 0.5148; % start at 0.5148 cells per L
b_c = b_n * R_CN;
...
% CO2 parameters
r0 = 0.18/(ts*86400); % dark respiration, sec-1 = 0.18 d-1, Geider & Osborne 1989 
b = 0.01; % respiration coefficient, no units, Geider & Osborne 1989
Kg = 0.4968/(ts*86400); % m sec-1 = 0.7 cm h-1 from Cole & Caraco 1998
...
B_n_v = zeros(ts*86400*md, 1); %preallocate
B_c_v = zeros(ts*86400*md, 1); 
X_v = zeros(ts*86400*md, 1); 
N_v = zeros(ts*86400*md, 1); 
C_v = zeros(ts*86400*md, 1); 
ON_v = zeros(ts*86400*md, 1); 
OC_v = zeros(ts*86400*md, 1);
mu_v = zeros(ts*86400*md, 1);
q_n_v = zeros(ts*86400*md, 1);
q_c_v = zeros(ts*86400*md, 1);
...
for i=1:ts*86400*md
    ...
    h=(.09/-27/(ts*86400))*i+.105; % the hight of the water columns in meters
    ...
    if i == ts*432000 && ts*950400 && ts*1555200
        C=C+1000;
    end
...
for k=1:1000
B_n_vv(k)=B_n_v(round(ts*86.4*md*k)); 
B_c_vv(k)=B_c_v(round(ts*86.4*md*k)); 
X_vv(k)=X_v(round(ts*86.4*md*k)); 
N_vv(k)=N_v(round(ts*86.4*md*k)); 
C_vv(k)=C_v(round(ts*86.4*md*k)); 
ON_vv(k)=ON_v(round(ts*86.4*md*k)); 
OC_vv(k)=OC_v(round(ts*86.4*md*k));
mu_vv(k)=mu_v(round(ts*86.4*md*k));
q_n_vv(k)=q_n_v(round(ts*86.4*md*k));
q_c_vv(k)=q_c_v(round(ts*86.4*md*k));
end
...
ydot9312 = mean(cell_data);
SD9312 = std(cell_data);
 				

Nx = [0	2	4	6	8	9	11	13	14	15	16	18	23];
N_data = [209.2235294	132.3294118	119.8117647	116.2352941	103.7176471	41.12941176	27.65803922	0	0	68.66823529	0	0	0;
              194.3215686	80.47058824	114.4470588	125.1764706	78.68235294	91.2	43.87137255	73.91372549	34.33411765	82.61647059	0	0	0;
              156.172549	82.25882353	118.0235294	155.5764706	121.6	44.10980392	82.02039216	75.64235294	21.45882353	15.03905882	0	0	0];
Ny = mean(N_data);
N_SD = std(N_data);
...
C_SD = std(C_data);
 ...
 plot(time,B_n_vv,'b'); 
 xlabel('Time (days)')
 ylabel('N biomass (umol N l-1)')
 title('Autotrophs N Biomass')
 
 subplot(3,3,2)
 plot(time,B_c_vv,'b'); 
 xlabel('Time (days)')
 ylabel('C biomass (umol C l-1)')
 title('Autotrophs C Biomass')
 ...
 xlabel('Time (days)')
 ylabel('Density (cells l-1)')
 title('Autotrophs Density')
 
 subplot(3,3,4)
 [yNaxes,autoline5,heteroline5]=plotyy(time,N_vv,time,ON_vv);
 xlabel('Time (days)')
 axes(yNaxes(1));
 ylabel('N concentration (umol l-1)')
 set(autoline5,'color','m')
 text(20,750,'Inorganic N')
 hold on
 errorbar(Nx,Ny,N_SD,'o','MarkerFaceColor','k')
 axes(yNaxes(2));
 ylabel('ON concentration (umol l-1)')
 set(heteroline5,'color','g')
 text(15,15,'Organic N')
 title('Inorganic and organic N concentration')
 
 subplot(3,3,5)
 [yCaxes,autoline6,heteroline6]=plotyy(time,C_vv,time,OC_vv);
 xlabel('Time (days)')
 axes(yCaxes(1));
 ylabel('C concentration (umol l-1)')
 set(autoline6,'color','m')
 text(23,30,'Inorganic C')
 axes(yCaxes(2));
 ylabel('OC concentration (umol l-1)')
 set(heteroline6,'color','g')
 text(10,-20,'Organic C')
 title('Inorganic and organic C concentration')

 subplot(3,3,7)
 plot(time,q_n_vv,'c',time,q_n_max,'c',time,q_n_min,'c')
 xlabel('Time (days)')
 ylabel('N quota (umol C cell-1)')
 title('Cell quotas')

 subplot(3,3,8)
 plot(time,q_c_vv,'r',time,q_c_max,'r',time,q_c_min,'r')
 xlabel('Time (days)')
 ylabel('C quota (umol C cell-1)')
 title('Cell quotas')

 subplot(3,3,6)
 plot(time,mu_vv,'b',time,mu_inf,'b'); 
 xlabel('Time (days)')
 ylabel('Growth rates (sec^{-1})')
 title('Auto growth rates')
...
title(titles{3})
ylabel('C (\mumol cell^{-1})')

subplot(2,3,4)
plot(time,q_n_vv)
title(titles{4})
ylabel('Q^N (\mumol L^{-1})')
xlabel('Time (days)')

subplot(2,3,5)
plot(time,q_c_vv)
title(titles{5})
ylabel('Q^C (\mumol L^{-1})')
xlabel('Time (days)')
...
results = [time; X_vv; N_vv; C_vv; ON_vv; OC_vv; B_n_vv; B_c_vv; q_n_vv; q_c_vv; q_c_vv./q_n_vv]; 
save TimeSteps_highVmax results -append -ascii 